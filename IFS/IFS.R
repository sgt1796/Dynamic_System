...
apply_functions <- function(data_list, functions) {
  ...
  for (func_name in names(functions)) {
    ...
    if (func_name %in% unique(data_list$mapping)) {
      ...
      func_params <- intersect(names(formals(func)), names(data_list)) # Get the parameter names of the function
      
      ## =========Debug output==================================================
      # var_summary = current_data %>%
      #   summarise(across(all_of(func_params), list(min = min, max = max))) %>%
      #   pivot_longer(
      #     cols = everything(),
      ...
      result <- current_data %>%
        rowwise() %>%
        mutate(new_coords = list(func(!!!rlang::syms(func_params)))) %>%
...
iterate_functions <- function(initial_points, iterations, mapping_list, report_per = FALSE) {
  ...
  for (iter in seq_len(iterations)) {
    ...
    ## ====== Option for a simple progress report ==============================
    if (report_per) {
      if (iter %% report_per == 0) {
        print(paste0("IFS working... (",iter,"/",iterations,")"))
      }
    }
    # ==========================================================================
